...
list.of.packages <- c("tidyverse","GGally","ggthemes","readxl","ggrepel","lme4","ppcor","stringr")
...
# save(iconicity,humor,valence,file="ratings.Rdata")
# if no connection:
...
df <- merge(iconicity,humor,by="word") %>%
  ...
  dplyr::select(word,iconicity,POS,humor,freq_log) %>%
  merge(valence,by="word") # add valence ratings
...
df %>%
  filter(diff_abs == 0) %>%
  arrange(desc(humor)) %>%
  dplyr::select(word,humor,iconicity,diff_abs) %>%
  sample_n(20)
...
df %>%
  ...
  ggplot(aes(iconicity,humor,color=POS)) +
  theme_tufte() + ggtitle("Humor and iconicity by POS") +
